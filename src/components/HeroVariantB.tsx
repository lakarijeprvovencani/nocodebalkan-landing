...
const HeroVariantB: React.FC = () => {
  ...
  const handleCTAClick = () => {
    ...
    if (typeof window !== 'undefined' && (window as any).fbq) {
      (window as any).fbq('track', 'Lead', {
        content_name: 'Hero CTA - Variant B',
  ...
  return (
    <div className="relative min-h-[50vh] md:min-h-screen bg-gray-900 flex items-center xl:items-end justify-center overflow-visible md:overflow-hidden pt-12 pb-4 md:py-16 xl:pb-48 z-[1]">
      ...
      <div id="hero-content" className="max-w-7xl 2xl:max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 2xl:px-16 relative z-10 transition-all duration-300">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-16 2xl:gap-24 items-center">
          {/* Left content */}
          <motion.div
          ...
          >
            <motion.p 
              className="text-purple-400 text-2xl md:text-4xl 2xl:text-5xl mb-4 md:mb-6 2xl:mb-8 italic"
            ...
            <motion.h1 
              className="text-4xl md:text-6xl lg:text-7xl 2xl:text-8xl font-bold text-white mb-4 md:mb-6 2xl:mb-8 md:tracking-tight"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: 0.2, duration: 0.8 }}
            >
              Zarađuj od 50 do 100 dolara
              <br className="md:hidden" /> <span style={{ color: '#FF0054' }}>na sat.</span>
            </motion.h1>
            <motion.p 
              className="text-lg md:text-3xl 2xl:text-4xl text-blue-300 mb-4 md:mb-8 2xl:mb-10"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          >
            <div className="relative w-full max-w-xl 2xl:max-w-2xl">
              <div className="grid grid-cols-2 gap-6 md:gap-8 2xl:gap-12 p-4 2xl:p-6">
                {[
                  ...
                  <motion.div
                    key={index}
                    className={`bg-gradient-to-br ${item.bgGradient} backdrop-blur-sm rounded-2xl p-8 2xl:p-12 flex flex-col items-center justify-center gap-4 2xl:gap-6 border border-gray-700/50 hover:border-gray-600 transition-colors`}
                  ...
                  >
                    ...
                    >
                      <item.Icon className={`w-16 h-16 2xl:w-24 2xl:h-24 ${item.iconColor}`} />
                    </motion.div>
                    <span className="text-white font-medium text-lg 2xl:text-2xl text-center">{item.label}</span>
                  </motion.div>
                ))}
              </div>
            </div>
